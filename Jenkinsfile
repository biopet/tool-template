node('local') {
    try {
        ...
        stage('Test') {
            sh "${tool name: 'sbt 0.13.15', type: 'org.jvnet.hudson.plugins.SbtPluginBuilder$SbtInstallation'}/bin/sbt -no-colors coverageOn test assembly coverageReport"
            sh "java -jar target/scala-2.11/*-assembly-*.*.*-SNAPSHOT.jar"
        }
