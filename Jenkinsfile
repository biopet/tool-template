node('local') {
    try {
        ...
        stage('Build & Test') {
            sh "${tool name: 'sbt 0.13.15', type: 'org.jvnet.hudson.plugins.SbtPluginBuilder$SbtInstallation'}/bin/sbt -no-colors clean biopetTest assembly | tee sbt.log"
            sh "java -jar target/scala-2.11/*-assembly-*.jar -h"
            sh 'n=`grep -ce "\\* org.scala-lang" sbt.log || true`; if [ "$n" -ne \"0\" ]; then echo "ERROR: Found conflicting dependencies inside biopet"; exit 1; fi'
